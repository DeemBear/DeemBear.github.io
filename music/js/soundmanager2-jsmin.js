...
(function(h, g) {
	function fa(fa, wa) {
		...
		var Ua, c = this,
			...
			ha = !1,
			p, sb = 0,
			X, B, Aa, O, Ba, M, P, Q, Xa, Ca, ia, I, ja, Da, R, Ea, Y, ka, la, S, Ya, Fa, Za = ["log", "info", "warn", "error"],
			$a, Ga, ab, Z = null,
			Ha = null,
			r, Ia, T, bb, ma, na, J, v, $ = !1,
			Ja = !1,
			cb, db, eb, oa = 0,
			aa = null,
			pa, N = [],
			qa, u = null,
			fb, ra, ba, K, sa, Ka, gb, w, hb = Array.prototype.slice,
			E = !1,
			La, H, Ma, ib, G, jb, Na, ta, kb = 0,
			ca = t.match(/(ipad|iphone|ipod)/i),
			lb = t.match(/android/i),
			L = t.match(/msie/i),
			tb = t.match(/webkit/i),
			ua = t.match(/safari/i) && !t.match(/chrome/i),
			Oa = t.match(/opera/i),
			ub = t.match(/firefox/i),
			Pa = t.match(/(mobile|pre\/|xoom)/i) || ca || lb,
			Qa = !U.match(/usehtml5audio/i) && !U.match(/sm2\-ignorebadua/i) && ua && !t.match(/silk/i) && t.match(/OS X 10_6_([3-7])/i),
			da = h.console !== g && console.log !== g,
			Ra = m.hasFocus !== g ? m.hasFocus() : null,
			va = ua && (m.hasFocus === g || !m.hasFocus()),
			mb = !va,
			nb = /(mp3|mp4|mpa|m4a|m4b)/i,
			ea = m.location ? m.location.protocol.match(/http/i) : null,
			ob = !ea ? "http://" : "",
			pb = /^\s*audio\/(?:x-)?(?:mpeg4|aac|flv|mov|mp4||m4v|m4a|m4b|mp4v|3gp|3g2)\s*(?:$|;)/i,
			qb = "mpeg4 aac flv mov mp4 m4v f4v m4a m4b mp4v 3gp 3g2".split(" "),
			vb = RegExp("\\.(" + qb.join("|") + ")(\\?.*)?$", "i");
		...
		this.setup = function(b) {
			...
			Aa(b);
			b && (d && (Y && b.url !== g) && c.beginDelayedInit(), !Y && (b.url !== g && "complete" === m.readyState) && setTimeout(R, 1));
			return c
		};
		...
		this.onready = function(b, d) {
			if ("function" === typeof b) q && c._wD(r("queue", "onready")), d || (d = h), Ba("onready", b, d), M();
			else throw r("needFunction",
				"onready");
			return !0
		};
		this.ontimeout = function(b, d) {
			if ("function" === typeof b) q && c._wD(r("queue", "ontimeout")), d || (d = h), Ba("ontimeout", b, d), M({
				type: "ontimeout"
			});
		...
		Ua = function(b) {
			...
			this.load = function(b) {
				...
				if (ra(e)) d = a._setup_html5(e), d._called_load ? c._wD(a.id + ": Ignoring request to load again") : (a._html5_canplay = !1, a.url !== e.url && (c._wD(p("manURL") + ": " + e.url), a._a.src =
					e.url, a.setPosition(0)), a._a.autobuffer = "auto", a._a.preload = "auto", a._a._called_load = !0, e.autoPlay && a.play());
				else {
				...
				if (a.paused && 0 <= a.position && (!a._iO.serverURL || 0 < a.position)) c._wD(e + "Resuming from paused state", 1), a.resume();
					...
					if (null !== a._iO.from && null !== a._iO.to && 0 === a.instanceCount && 0 === a.playState && !a._iO.serverURL) {
						...
						if (a.isHTML5 && !a._html5_canplay) c._wD(e + "Beginning load for from/to case"), a.load({
							...
							x = !1;
						else if (!a.isHTML5 && !a.loaded && (!a.readyState || 2 !== a.readyState)) c._wD(e + "Preloading for from/to case"), a.load({
							onload: f
						}), x = !1;
						if (null !== x) return x;
						a._iO = y()
			...
			this.clearOnPosition = function(a, b) {
				...
				for (c = 0; c < D.length; c++)
					if (a === D[c].position && (!b || b === D[c].method)) D[c].fired && t--, D.splice(c, 1)
			};
			...
			this._setup_html5 = function(b) {
				...
				if (c) {
					if (c._s)
						...
						else if (!E && d === decodeURI(u)) return a._apply_loop(c, b.loops), c;
					e || (f(!1), c.src =
		...
		B = function(b, d) {
			...
			for (f in a) a.hasOwnProperty(f) && e[f] === g && (e[f] = "object" !==
				typeof a[f] || null === a[f] ? a[f] : B(e[f], a[f]));
			return e
		};
		ta = function(b, c) {
		...
		Aa = function(b, d) {
			...
			for (e in b)
				if (b.hasOwnProperty(e))
					if ("object" !==
						typeof b[e] || null === b[e] || b[e] instanceof Array || b[e] instanceof RegExp) f && O[d] !== g ? c[d][e] = b[e] : x[e] !== g ? (c.setupOptions[e] = b[e], c[e] = b[e]) : O[e] === g ? (J(r(c[e] === g ? "setupUndef" : "setupError", e), 2), a = !1) : c[e] instanceof Function ? c[e].apply(c, b[e] instanceof Array ? b[e] : [b[e]]) : c[e] = b[e];
		...
		G = {
			...
			progress: s(function(b) {
				...
				if (z && z.length) {
					e = 0;
					for (a = z.length; e < a; e++) d.buffered.push({
						start: 1E3 * z.start(e),
						end: 1E3 * z.end(e)
					});
		...
		ra = function(b) {
			return !b ||
				!b.type && !b.url && !b.serverURL ? !1 : b.serverURL || b.type && ga(b.type) ? !1 : b.type ? ba({
					type: b.type
				}) : ba({
					url: b.url
		...
		gb = function() {
			function b(a) {
				...
				if (a instanceof Array) {
					...
					for (e = a.length; b < e; b++)
						if (c.html5[a[b]] || d.canPlayType(a[b]).match(c.html5Test)) f = !0, c.html5[a[b]] = !0, c.flash[a[b]] = !!a[b].match(nb);
					b = f
			...
			var d = Audio !== g ? Oa && 10 > opera.version() ? new Audio(null) : new Audio : null,
				e, a, f = {},
				h;
			h = c.audioFormats;
			for (e in h)
				if (h.hasOwnProperty(e) && (a = "audio/" + e, f[e] = b(h[e].type), f[a] = f[e], e.match(nb) ? (c.flash[e] = !0, c.flash[a] = !0) : (c.flash[e] = !1, c.flash[a] = !1), h[e] && h[e].related))
					for (a = h[e].related.length -
		...
		Ma = function() {
			...
			else if (f !== g && !t.match(/MSAppHost/i)) {
				try {
		...
		fb = function() {
			...
			if (ca && t.match(/os (1|2|3_0|3_1)/i)) c.hasHTML5 = !1, c.html5Only = !0, c.oMC && (c.oMC.style.display = "none");
		...
		la = function(b,
			...
			var f = d || c.url,
				...
				p, s, q, n = n && n.dir && n.dir.match(/rtl/i);
			b = b === g ? c.id : b;
			Ca();
			c.url = ab(ea ? f : h);
			d = c.url;
			...
			if (L) f = m.createElement("div"), s = ['\x3cobject id\x3d"' + b + '" data\x3d"' + d + '" type\x3d"' + k.type + '" title\x3d"' + k.title + '" classid\x3d"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase\x3d"' + ob + 'download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version\x3d6,0,40,0"\x3e', a("movie", d), a("AllowScriptAccess", c.allowScriptAccess), a("quality", k.quality), c.wmode ? a("wmode", c.wmode) : "", a("bgcolor", c.bgColor), a("hasPriority", "true"), c.debugFlash ?
				a("FlashVars", k.FlashVars) : "", "\x3c/object\x3e"
			].join("");
			else
			...
			l = T();
			if (k = ka())
				if (c.oMC = A(c.movieID) || m.createElement("div"), c.oMC.id) q = c.oMC.className, c.oMC.className = (q ? q + " " : "movieContainer") + (l ? " " + l : ""), c.oMC.appendChild(f), L && (p = c.oMC.appendChild(m.createElement("div")), p.className = "sm2-object-box", p.innerHTML = s), W = !0;
				else {
					c.oMC.id = c.movieID;
		...
		X = function(b) {
			...
			var d = !0,
				e;
			if (!c.useFlashBlock || !c.flashLoadTimeout || c.getMoviePercent()) q = !0, y && (e = {
		...
		R = function() {
			...
			var b = null,
				b = null,
				d = U.toLowerCase(); - 1 !== d.indexOf("sm2-usehtml5audio\x3d") && (b = "1" === d.charAt(d.indexOf("sm2-usehtml5audio\x3d") + 18), da && console.log((b ? "Enabling " : "Disabling ") +
				"useHTML5Audio via URL parameter"), c.setup({
				useHTML5Audio: b
			})); - 1 !== d.indexOf("sm2-preferflash\x3d") && (b = "1" === d.charAt(d.indexOf("sm2-preferflash\x3d") + 16), da && console.log((b ? "Enabling " : "Disabling ") + "preferFlash via URL parameter"), c.setup({
				preferFlash: b
			}));
			...
			gb();
			!H && u && (N.push(I.needFlash), c.setup({
				flashLoadTimeout: 1
			}));
